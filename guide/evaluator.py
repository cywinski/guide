...
def main():
    ...
    parser.add_argument(
        "--ref_batch", help="path to reference batch npz acts file", required=False
    )
    parser.add_argument(
        "--ref_acts", help="path to reference batch npz acts file", required=False
    )
    ...
    if args.ref_acts is None:
        ref_acts = evaluator.read_activations(args.ref_batch)
    # ref_acts = evaluator.read_activations(args.ref_batch)
    else:
        ref_acts = (
            np.load(args.ref_acts)["arr_0"],
            np.load(args.ref_acts)["arr_1"],
        )
