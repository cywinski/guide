...
class CacheClassLabel(data.Dataset):
    ...
    def __init__(self, dataset, target_transform=None):
        ...
        self.labels = torch.zeros(len(dataset)).long()
        if target_transform:
            self.labels = target_transform(self.labels)
        label_cache_filename = path.join(
            dataset.root, str(type(dataset)) + "_" + str(len(dataset)) + ".pth"
        )
        ...
        if target_transform:
            self.number_classes = self.labels.shape[1]
...
class AppendName(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.return_task_as_class:
            out_dict["y"] = np.array(
                self.task_ids[index]
            )  # np.array(self.task_id, dtype=np.int64)
        elif self.return_classes:
            if isinstance(target, torch.Tensor):
                out_dict["y"] = target.cpu()
            elif isinstance(target, int):
                out_dict["y"] = torch.from_numpy(np.array(target, dtype=np.int64))
...
class Subclass(data.Dataset):
    ...
    def __init__(self, dataset, class_list, remap=True):
        ...
        ), "dataset must be wrapped by CacheClassLabel"
        self.dataset = dataset
