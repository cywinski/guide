...
from .layers import ConvBlock, FinalBlock, InitialBlock
...
class ResNet(nn.Module):
    def __init__(self, opt):
        ...
        self.num_classes = opt.model_num_classes
        ...
        self.noised = opt.noised

        in_size = (
            out_planes * block.expansion + 1
            if opt.noised
            else out_planes * block.expansion
        )
        self.final = FinalBlock(opt=opt, in_channels=in_size)
    ...
    def forward(self, x, t=None, return_h=False):
        out = self.initial(x)
        out = self.group1(out)
        out = self.group2(out)
        out = self.group3(out)
